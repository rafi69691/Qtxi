...
class QuotexLiveWebSocket:
    ...
    def ingest_live_tick(self, asset: str, price: float, timestamp: float = None):
        """
        Receives real-time ticks directly from Kiwi Browser Live Bridge.
        Guarantees 100% identical price & candles to Quotex trading screen.
        """
        clean_asset = asset.upper().replace("_", "-").replace(" ", "-")
        self._process_tick(clean_asset, float(price), timestamp or time.time())
