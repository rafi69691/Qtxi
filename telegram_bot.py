...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    welcome_text = (
        ...
        f"🔥 **ফিচারসমূহ:**\n"
        f"• Quotex OTC লাইভ স্ক্রিন সিঙ্কড ফিড\n"
...
# --- Built-in HTTP Live Bridge Server for Kiwi Browser ---
...
async def handle_bridge_feed(request):
    try:
        ...
        if asset and price:
            feed.ingest_live_tick(asset, price, ts)
            return web.json_response(
                {"status": "ok", "asset": asset, "price": price},
...
async def start_web_bridge(app_instance):
    web_app = web.Application()
    web_app.router.add_options('/feed', handle_cors_options)
    web_app.router.add_post('/feed', handle_bridge_feed)
    web_app.router.add_get('/', lambda r: web.Response(text="⚡ XT AI PRO Quotex Live Bridge Online ⚡", headers={"Access-Control-Allow-Origin": "*"}))
    ...
    await site.start()
    logger.info(f"[+] Quotex Live Bridge HTTP Server listening on port {port}...")
